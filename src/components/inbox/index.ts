// Barrel export for inbox components.
// Modular components extracted from the monolithic Inbox.tsx.

// Existing modular components
export { InboxCaixaRail } from "./InboxCaixaRail";
export { ConversationTasks } from "./ConversationTasks";
export { InboxTasksModal } from "./InboxTasksModal";
export { RecordTasks } from "./RecordTasks";
export { EmojiPicker } from "./EmojiPicker";
export { FileTypeIcon, getFileMeta } from "./FileTypeIcon";
export { ProgressDownloadButton } from "./ProgressDownloadButton";
export { MediaViewer } from "./MediaViewer";
export type { MediaItem } from "./MediaViewer";
export { WaveformPlayer } from "./WaveformPlayer";
export { VideoPlayer } from "./VideoPlayer";
export { GlobalAudioPill } from "./GlobalAudioPill";
