  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-[1fr_120px_auto] gap-3 items-end">
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">Servidor</Label>
            <Input placeholder="imap.seudominio.com" value={form.imap_server} onChange={(e) => update({ imap_server: e.target.value })} />
          </div>
          <div className="space-y-1.5">
        ...
        <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <Lock className="h-3 w-3" /> Utilizador IMAP
            </Label>
            <Input
              placeholder={form.email_address || 'utilizador@email.com'}
              value={form.imap_login}
              onChange={(e) => update({ imap_login: e.target.value })}
              autoComplete="new-password"
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <Lock className="h-3 w-3" /> Password IMAP
            </Label>
            <PasswordInput
      ...
      <div className="rounded-xl border overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-[1fr_120px_auto] gap-3 items-end">
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">Servidor</Label>
            <Input placeholder="smtp.seudominio.com" value={form.smtp_server} onChange={(e) => update({ smtp_server: e.target.value })} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">Porta</Label>
        ...
        <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <Lock className="h-3 w-3" /> Utilizador SMTP
            </Label>
            <Input
              placeholder={form.email_address || 'utilizador@email.com'}
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <Lock className="h-3 w-3" /> Password SMTP
            </Label>
            <PasswordInput
...
function EmailFullScreenEditor({
  ...
  const handleSave = () => {
    setFormError(null);
    if (isEdit) {
      const payload: EmailChannelUpdate = {
      ...
      const payload: EmailChannelInput = {
  ...
  const tabs: { key: EditorTab; label: string }[] = [
    { key: 'config', label: 'Configuração da conta' },
    { key: 'signatures', label: 'Assinaturas da conta' },
  ];
    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center gap-3 pb-4 border-b shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="font-bold text-sm leading-tight truncate">
            {isEdit ? (channel!.label ?? 'Caixa de Email') : 'Nova Caixa de Email'}
          </h2>
          {isEdit && channel!.metadata?.email_address && (
            <p className="text-xs text-muted-foreground truncate">{channel!.metadata.email_address}</p>
          )}
        </div>
      </div>
      ...
      {activeTab === 'config' && (
        <div className="flex gap-2 pt-4 border-t shrink-0">
          <Button onClick={handleSave} disabled={!canSubmit} className="flex-1">
...
// --- AddEmailModal (used by IntegrationsContent) ---
...
// --- EditEmailModal (used by IntegrationsContent) ---
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-muted/20 p-4 space-y-2">
        ...
        <p className="text-xs text-amber-600 dark:text-amber-400">
          As credenciais são guardadas de forma segura e utilizadas pelo servidor para fazer polling IMAP. Cada conta criada aqui equivale a uma caixa independente.
        </p>
      </div>
    </div>
  );
