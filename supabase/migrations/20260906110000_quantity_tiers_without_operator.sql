-- ============================================================
-- Escalões de quantidade em qualquer produto, com ou sem operadora
-- ============================================================
-- Substitui 20260906100000. Os escalões só eram lidos se o produto
-- estivesse ligado a uma operadora COM commission_basis definido — na
-- prática, só a Digi. Um produto sem operadora (1P ou 2P, os Níveis) podia
-- ter escalões configurados no ecrã e o servidor ignorava-os em silêncio.
--
-- Passa a ser simples: há escalões, usam-se. O commission_basis da
-- operadora decide apenas COMO se resolve a banda —
--   'monthly_volume' -> pelo volume acumulado do mês (Digi)
--   tudo o resto      -> pela quantidade desta venda
--
-- Nada muda no que já existe: hoje os únicos produtos com escalões são os
-- da Digi, e esses continuam a resolver pelo volume mensal.
