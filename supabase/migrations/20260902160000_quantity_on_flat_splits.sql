-- Quantity now applies to EVERY catalog product, not just the ones with
-- quantity_tiers: a sale can carry three of the same package. The only
-- calculation that had not caught up was the flat (non-tiered) fixed split,
-- which paid its rule once no matter how many units were sold, so the sale
-- screen showed 3x what the server actually froze. Everything else in this
-- function is unchanged from 20260902130000_energia_fixed_commission.sql.
