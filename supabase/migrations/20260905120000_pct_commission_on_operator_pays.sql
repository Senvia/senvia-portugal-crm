-- ============================================================
-- Comissão em % passa a ser % do que a OPERADORA paga
-- ============================================================
-- Substitui 20260905110000. Uma linha em % era lida como percentagem do
-- preço do produto — o que o CLIENTE paga. Em telecom o cliente não paga
-- nada à org: o preço destes produtos é 0 e a percentagem dava sempre 0€.
--
-- A base da percentagem passa a ser o que a operadora paga à org por
-- unidade (do escalão, senão do produto, na tecnologia instalada). Sem
-- operator_pays configurado — todos os produtos não-telecom — cai no preço
-- unitário, exatamente como antes.
...
BEGIN
  ...
  FOREACH _produto IN ARRAY _produtos LOOP
    ...
    IF _seller_split IS NOT NULL THEN
      ...
      -- Uma percentagem é uma fatia do que a operadora paga, não do preço
      -- ao cliente. Sem operadora configurada, cai no preço unitário.
      _seller_rate := CASE
        WHEN _split_type = 'pct'
          THEN ROUND(COALESCE(_operator_pays, _unit_price) * _split_val / 100.0, 2)
        ELSE _split_val
      END;
    END IF;
