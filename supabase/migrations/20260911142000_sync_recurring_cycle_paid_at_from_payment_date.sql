      ...
      SET
        ...
        paid_at = CASE
          WHEN NEW.status = 'paid'
            THEN NEW.payment_date::timestamp AT TIME ZONE 'Europe/Lisbon'
          ELSE NULL
        END
      WHERE id = v_cycle_id;
    END IF;
...
AFTER INSERT OR UPDATE OF status, payment_date, recurring_cycle_id OR DELETE
ON public.sale_payments
FOR EACH ROW
...
WHERE payment.recurring_cycle_id = cycle.id
  AND (
    ...
    OR cycle.paid_at IS DISTINCT FROM CASE
      WHEN payment.status = 'paid'
        THEN payment.payment_date::timestamp AT TIME ZONE 'Europe/Lisbon'
      ELSE NULL
    END
