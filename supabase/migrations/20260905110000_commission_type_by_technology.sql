-- ============================================================
-- Comissão por tecnologia: valor E tipo (€ / %) independentes
-- ============================================================
-- Substitui 20260905100000. Além do valor, cada tecnologia passa a ter o
-- seu próprio modo: a mesma operadora paga muitas vezes um valor fixo em
-- fibra e uma percentagem em satélite, e até aqui o €/% era um só para a
-- linha inteira.
--
-- Resolução, sempre com recurso ao valor único:
--   comissão do vendedor -> split.value_<tech>       senão split.value
--   modo (€ / %)         -> split.type_<tech>        senão split.type
--   operadora paga       -> tier.operator_pays_<tech> senão tier.operator_pays
--                           senão produto.operator_pays_<tech> senão .operator_pays
--
-- Sem tecnologia na linha (todas as vendas até hoje) nada muda: cai no valor
-- e no modo únicos, e o resultado é bit a bit o mesmo de antes.
    ...
    IF (_details->_produto) ? 'total_cards' THEN
      _extra_qty := GREATEST(0,
        COALESCE(public._safe_numeric(_details->_produto->>'total_cards'), 0)
          - COALESCE(public._safe_numeric(_cat_entry->>'included_cards'), 1)
      );
    ELSE
    ...
    IF _seller_split IS NOT NULL THEN
      ...
      _seller_rate := CASE
        WHEN _split_type = 'pct'
          THEN ROUND(_unit_price * _split_val / 100.0, 2)
        ELSE _split_val
      END;
    END IF;
